...
export default function FarmsManager() {
  ...
  const handleDelete = async (farmId: string) => {
    ...
    // Vérifier d'abord s'il y a des produits associés pour informer l'utilisateur
    let productCount = 0;
    try {
    ...
    // Message de confirmation adapté selon qu'il y a des produits ou non
    const confirmMessage = productCount > 0 
      ? `Êtes-vous sûr de vouloir supprimer "${farmName}" ?\n\n⚠️ ${productCount} produit(s) seront désassociés de cette ferme.\nLa ferme sera définitivement supprimée.`
      : `Êtes-vous sûr de vouloir supprimer "${farmName}" ?`;
    ...
    if (confirm(confirmMessage)) {
      try {
        ...
        if (response.ok) {
          ...
          notifyAdminUpdate('farms', 'delete', { id: farmId });
          ...
          const successMsg = document.createElement('div');
          successMsg.className = 'fixed top-4 right-4 bg-green-600 text-white px-6 py-3 rounded-lg shadow-lg z-[9999]';
          successMsg.textContent = productCount > 0 
            ? `✅ "${farmName}" supprimée avec succès! (${productCount} produit(s) désassocié(s))`
            : `✅ "${farmName}" supprimée avec succès!`;
          document.body.appendChild(successMsg);
