...
export default function Cart() {
  ...
  const handleConfirmOrder = async () => {
    ...
    // Construire le message avec le nouveau format amélioré
    ...
    items.forEach((item, index) => {
      ...
      if (item.discount > 0) {
        message += `   • Remise: 🏷️ -${item.discount}%\n`;
      }
    ...
    message += `🕐 *Commande passée le:* ${dateStr} à ${timeStr}`;
    ...
    // Copier le message dans le presse-papiers comme sauvegarde
    try {
      await navigator.clipboard.writeText(message);
      console.log('📋 Message copié dans le presse-papiers');
    } catch (error) {
      console.log('❌ Impossible de copier dans le presse-papiers:', error);
    }
