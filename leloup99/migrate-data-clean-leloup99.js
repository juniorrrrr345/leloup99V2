  ...
  try {
    ...
    for (const farm of uniqueFarms) {
      await executeSqlOnD1(
        ...
        [farm.name || 'Farm', 'Production LELOUP99', farm.location || farm.country || 'Local', 'contact@leloup99.com']
      );
    }
    ...
    for (const product of uniqueProducts) {
      ...
      await executeSqlOnD1(
        ...
        [
          ...
          product.description || 'Produit LELOUP99 de qualité',
          category_id,
          farm_id,
          product.image || '',
          product.video || '',
    ...
    // Migration settings (UPDATE si existe, INSERT sinon)
    try {
      await executeSqlOnD1(
        'INSERT INTO settings (id, shop_title, theme_color, background_opacity, background_blur) VALUES (?, ?, ?, ?, ?)',
        [1, 'LELOUP99', 'glow', 20, 5]
      );
      console.log('✅ Settings LELOUP99 créés');
    } catch (error) {
      // Si déjà existant, on met à jour
      await executeSqlOnD1(
        'UPDATE settings SET shop_title = ?, theme_color = ?, background_opacity = ?, background_blur = ? WHERE id = 1',
        ['LELOUP99', 'glow', 20, 5]
      );
      console.log('✅ Settings LELOUP99 mis à jour');
    }
